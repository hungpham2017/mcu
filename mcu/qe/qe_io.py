...
def read_pw_output(filename):  
    ...
    with open(filename, "r") as data_file:
        ...
        atom_type = []
        atom_position = []
        for atom in atom_MATCH.finditer(atoms_block):
            atom_dict = atom.groupdict()
            atom_type.append(atom_dict['species'])
        ...
        out['atom_position'] = atom_position
        out['kpts'] = kpts
        out['eigenvals'] = band
        out['hoco'] = hoco 
        out['luco'] = luco  
...
def read_dos_output(filename):  
    '''Read the band generated by dos.x'''
    ...
    with open(filename, "r") as data_file:
        ...
        if dos_header is not None:
            ...
            dos = np.float64(' '.join(data[1:]).split()).reshape(-1,3)
        else:
            dos_header = dos_polarized_header_MATCH.match(data[0])
            efermi = float(dos_header['efermi'])
            dos = np.float64(' '.join(data[1:]).split()).reshape(-1,4)
...
tot_pdos            = "# ik    E (eV)  dos(E)    pdos(E)"
tot_pdos_polarized  = "# ik    E (eV)  dosup(E)   dosdw(E)  pdosup(E)  pdosdw(E)"
...
def wfc_match(num_ao):
    pattern = ['''(?P<c''' + str(i) + '''>\S+)\*[# [ ]* (?P<ao''' + str(i) + '''>\d+)]\+''' for i in range(num_ao)]
    wfc_MATCH = re.compile(r'''[ ]* ''' + ''.join(pattern), re.VERBOSE)
    return wfc_MATCH
    ...
    with open(filename, "r") as data_file:
        ...
        for wfc_kpt in wfc_kpts_MATCH.finditer(data):
            ...
            for wfc_e in wfc_e_MATCH.finditer(wfc_kpts + '\n===='):
                ...
                num_ao = wfc.count("#")
                temp = wfc_match(num_ao).match(wfc).groupdict().values()
